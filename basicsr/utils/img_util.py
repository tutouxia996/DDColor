...
def imread(file_path, flag='color'):
    """Read an image from file, with Unicode path support (Windows-safe).
    ...
    with open(file_path, 'rb') as f:
        data = f.read()
    return cv2.imdecode(np.frombuffer(data, dtype=np.uint8), imread_flags[flag])


def imwrite(img, file_path, params=None, auto_mkdir=True):
    """Write image to file, with Unicode path support (Windows-safe).
